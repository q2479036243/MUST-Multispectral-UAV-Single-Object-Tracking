...
class UNTrackActor(BaseActor):
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        for i in range(len(pred_dict)):
            # get GT
            gt_bbox = gt_dict['search_anno'][i+self.cfg.DATA.SEARCH.LENGTH-1]  # (Ns, batch, 4) (x1,y1,w,h) -> (batch, 4)
            gt_gaussian_maps = gt_gaussian_maps_list[i+self.cfg.DATA.SEARCH.LENGTH-1].unsqueeze(1)
