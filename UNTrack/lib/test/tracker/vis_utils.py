...
def gen_masked_tokens(tokens, indices, alpha=0.2):
    ...
    ###
    filtered_indices = indices[indices >= len(tokens)].reshape(1, -1)
    indices = filtered_indices[0].astype(int) - len(tokens)
    ###
...
def recover_image(tokens, H, W, Hp, Wp, patch_size):
    # image: (C, 196, 16, 16)
    image = tokens.reshape(Hp, Wp, patch_size, patch_size, 8).swapaxes(1, 2).reshape(H, W, 8)
    return image
...
def gen_visualization(image, mask_indices, patch_size=16):
    ...
    image_tokens = image.reshape(Hp, patch_size, Wp, patch_size, 8).swapaxes(1, 2).reshape(Hp * Wp, patch_size, patch_size, 8)
