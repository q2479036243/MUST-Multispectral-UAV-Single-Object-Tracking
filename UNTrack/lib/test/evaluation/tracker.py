...
def trackerlist(name: str, parameter_name: str, dataset_name: str, run_ids = None, num_searches: int = None, display_name: str = None,
                result_only=False):
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, dataset_name, run_id, num_searches, display_name, result_only) for run_id in run_ids]
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, num_searches: int = None, display_name: str = None,
                 result_only=False):
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num in range(1, len(seq.frames)):
            images = []
            for ind in range(self.num_searches, 0, -1):
                frame_path = seq.frames[max(frame_num - ind + 1, 0)]
    ...
    def _read_image(self, image_file: str):
        ...
        elif isinstance(image_file, list) and len(image_file) == 3:
            # im1 = cv.imread(image_file[0])
            # im2 = cv.imread(image_file[1])
            # im3 = cv.imread(image_file[2])
            # im = np.concatenate((im1, im2, im3[:,:,:2]), axis=2)
            im1 = jpeg4py.JPEG(image_file[0]).decode()
            im2 = jpeg4py.JPEG(image_file[1]).decode()
            im3 = jpeg4py.JPEG(image_file[2]).decode()
            im = np.concatenate((im1[:,:,2:3], im1[:,:,1:2], im1[:,:,0:1], im2[:,:,2:3], im2[:,:,1:2], im2[:,:,0:1], im3[:,:,1:2], im3[:,:,0:1]), axis=2)
            return im
        elif isinstance(image_file, list) and len(image_file) == 6:
            im1 = jpeg4py.JPEG(image_file[0]).decode()
            im2 = jpeg4py.JPEG(image_file[1]).decode()
            im3 = jpeg4py.JPEG(image_file[2]).decode()
            im4 = jpeg4py.JPEG(image_file[3]).decode()
            im5 = jpeg4py.JPEG(image_file[4]).decode()
            im6 = jpeg4py.JPEG(image_file[5]).decode()
